...
foreach ($items as $i => &$item) {
    ...
    if (isset($item->content) AND $item->content) {
        echo '<li class="maximenuck maximenuflatlistck '. $item->classe . ' level' . $itemlevel .' '.$item->liclass . '" data-level="' . $itemlevel . '">' . $item->content;
		$item->ftitle = '';
    }
    ...
    if ($item->ftitle != "") {
        ...
        if ($params->get('imageonly', '0') == '1')
            $item->ftitle = '';
        echo '<li class="maximenuck maximenuflatlistck '. $item->classe . ' level' . $itemlevel .' '.$item->liclass . '" style="z-index : ' . $zindex . ';" data-level="' . $itemlevel . '">';
        switch ($item->type) :
            default:
