        ...
        <div class="maxiroundedcenter">
            <ul class="<?php echo $params->get('moduleclass_sfx'); ?> maximenuck" style="position:relative;" >
				...
				foreach ($items as $i => &$item) {
					...
					$stopdropdownclass = ( $item->level > 1 ) ? ' nodropdown' : '';
					if ($item->level > 1) {
					...
					if ($item->ftitle != "") {
						...
						echo '<li data-level="' . $itemlevel . '" class="maximenuck' . $stopdropdownclass . $item->classe . ' level' . $itemlevel . ' ' . $item->liclass . ' fullwidth" style="z-index : ' . $zindex . ';position:static;">';
						switch ($item->type) :
							default:
					...
					if ($item->deeper) {
						...
						if ( $item->level == 1 && $params->get('orientation', 'horizontal') == 'horizontal' ) {
							$item->styles .= "position:absolute;left:0;right:0;";
							$item->innerstyles .= "width:auto;";
						} else if ( $item->level == 1 && $params->get('orientation', 'horizontal') == 'vertical' ) {
